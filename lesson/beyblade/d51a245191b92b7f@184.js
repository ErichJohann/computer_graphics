import define1 from "./448b7bb92306a565@375.js";
import define2 from "./7dfaa2667a586243@56.js";
...
function _pontos_bezier(curva){return(
function pontos_bezier() {
  return curva.vertex
}
)}
...
function _nuvemPontos(pontos_bezier){return(
function nuvemPontos(f,m,n) {
  m = m || 50;
  n = n || 100;
  let temp = pontos_bezier()
  let len = temp.length
  let pontos = new Float32Array((len + len/2)*n);
  let k = 0;
  for(let i = 0; i < len/2; i++) {
    
    for(let j = 0; j < n; j++) {
      let theta = 2*Math.PI*j/(n-1)
      pontos[k++] = temp[2*i] * Math.cos(theta);
      pontos[k++] = temp[2*i+1];
      pontos[k++] = temp[2*i] * Math.sin(theta);
    }
  }
  return pontos;
}
...
export default function define(runtime, observer) {
  ...
  main.import("editorBezier", child1);
  ...
  main.variable(observer("pontos_bezier")).define("pontos_bezier", ["curva"], _pontos_bezier);
  main.variable(observer("position")).define("position", ["nuvemPontos","esfera"], _position);
  main.variable(observer("nuvemPontos")).define("nuvemPontos", ["pontos_bezier"], _nuvemPontos);
  ...
  main.import("viewHandler", child2);
  return main;
}
